...
public interface CourseRepository extends JpaRepository<Course, Long> {
    ...
    /**
      ...
      */
    /*
    @Query(value = "SELECT COURSES.ID, COURSES.NAME, SUBJECTS.ID AS subjectId, SUBJECTS.NAME AS subjectName FROM COURSES INNER JOIN STUDENTS_COURSES " +
           "INNER JOIN STUDENTS INNER JOIN STUDENTS_SUBJECTS INNER JOIN SUBJECTS INNER JOIN TEACHERS_SUBJECTS " +
           "ON COURSES.ID = STUDENTS_COURSES.COURSE_ID AND STUDENTS_COURSES.STUDENT_ID = STUDENTS.ID " +
           "AND STUDENTS.ID = STUDENTS_SUBJECTS.STUDENT_ID AND STUDENTS_SUBJECTS.SUBJECT_ID = SUBJECTS.ID " +
           "AND SUBJECTS.ID = TEACHERS_SUBJECTS.SUBJECT_ID AND TEACHERS_SUBJECTS.TEACHER_ID= :id " +
           "AND COURSES.YEAR_OF_GENERATION= :year ORDER BY COURSES.ID", nativeQuery = true)
